...
main(const int, char**)
{
    try
    {
        ...
        vca::init_logging(work_dir / vca::Path{"logs"} /
                          vca::Path{"findled.log"});
        ...
        vca::UserConfig user_config{commands,
                                    work_dir / vca::Path{"user.json"}};

        vca::SqliteUserDb user_db{work_dir / vca::Path{"user.db"},
